...
tweets_cleav3_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav3 <- rbind(y, twitter_cleav3)
}

frq(twitter_cleav3$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav3 <- twitter_cleav3 %>% select(-cleavage_3_10, -cleavage_3)

# Create DFM  -----------------------------------------------------------------
twitter_cleav3 <- corpus(twitter_cleav3)

# Further cleaning procedure
twitter_cleav32 <- tokens(twitter_cleav3 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav32 <- tokens_remove(twitter_cleav32, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav32 <- tokens_remove(twitter_cleav32, c("0*"))
twitter_cleav32 <- tokens_wordstem(twitter_cleav32, language = "german")
twitter_cleav32 <- dfm(twitter_cleav32)
topfeatures(twitter_cleav32 , 1000)  # Check Topfeatures and in how many docs they are

# Trimming 1: Remove features that appear in less than 15 Tweets
twitter_cleav32 <- dfm_trim(twitter_cleav32 , min_docfreq = 15, verbose=TRUE)
twitter_cleav32  <- dfm_remove(twitter_cleav32 , min_nchar = 2)
topfeatures(twitter_cleav32 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav32[ntoken(twitter_cleav32) == 0,]
twitter_cleav32  <-  twitter_cleav32[ntoken(twitter_cleav32) != 0,]
twitter_cleav32[ntoken(twitter_cleav32) == 0,]



twitter_cleav3_df <- twitter_cleav32 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav32)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav3_df) <- make.names(names(twitter_cleav3_df))

frq(twitter_cleav3_df$ideology_scale7)
tweets_cleav3_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav3_df, target = "ideology_scale7")
...
export(conf_matrix_knn, file = "TwCl3_confmatrix_knn.xlsx")
...
capture.output(bayesCV, file = "TwCl3_NB.txt")
...
export(conf_matrix_NB, file = "TwCl3_confmatrix_NB.xlsx")
...
capture.output(cvWithTuning, file = "TwCl3_SVM.txt")
...
export(conf_matrix_SVM, file = "TwCl3_confmatrix_SVM.xlsx")
...
cv_RF <- resample(learner = forest, task = ml_task, par.vals = tunedForestPars$x,
                  resampling = cv, measures = list(mmce, acc))
capture.output(cv_RF, file = "TwCl3_RF.txt")
...
export(conf_matrix_RF, file = "TwCl3_confmatrix_RF.xlsx")

# Export Frequencies ------------------------------------------------------
library(rio)
export(tweets_cleav3_doc_7, "tweets_cleav3_finalDocs.xlsx")
export(tweets_cleav3_fea_7, "tweets_cleav3_DocsPerCat.xlsx")
