...
tweets_cleav5_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav5 <- rbind(y, twitter_cleav5)
}

frq(twitter_cleav5$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav5 <- twitter_cleav5 %>% select(-cleavage_5_10, -cleavage_5)

# Create DFM  -----------------------------------------------------------------
twitter_cleav5 <- corpus(twitter_cleav5)

# Further cleaning procedure
twitter_cleav52 <- tokens(twitter_cleav5 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav52 <- tokens_remove(twitter_cleav52, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav52 <- tokens_remove(twitter_cleav52, c("0*"))
twitter_cleav52 <- tokens_wordstem(twitter_cleav52, language = "german")
twitter_cleav52 <- dfm(twitter_cleav52)
topfeatures(twitter_cleav52 , 1000)  # Check Topfeatures and in how many docs they are

# Trimming 1: Remove features that appear in less than 95 Tweets
twitter_cleav52 <- dfm_trim(twitter_cleav52 , min_docfreq = 15, verbose=TRUE)
twitter_cleav52  <- dfm_remove(twitter_cleav52 , min_nchar = 2)
topfeatures(twitter_cleav52 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav52[ntoken(twitter_cleav52) == 0,]
twitter_cleav52  <-  twitter_cleav52[ntoken(twitter_cleav52) != 0,]
twitter_cleav52[ntoken(twitter_cleav52) == 0,]



twitter_cleav5_df <- twitter_cleav52 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav52)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav5_df) <- make.names(names(twitter_cleav5_df))

frq(twitter_cleav5_df$ideology_scale7)
tweets_cleav5_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav5_df, target = "ideology_scale7")
...
capture.output(cv_knn, file = "TwCl5_knn.txt")
...
export(conf_matrix_knn, file = "TwCl5_confmatrix_knn.xlsx")
...
export(knn_1_fold, "pred_TwCl5_knn1.xlsx")
export(knn_2_fold, "pred_TwCl5_knn2.xlsx")
export(knn_3_fold, "pred_TwCl5_knn3.xlsx")
...
capture.output(bayesCV, file = "TwCl5_NB.txt")
...
export(conf_matrix_NB, file = "TwCl5_confmatrix_NB.xlsx")
...
export(nb_1_fold, "pred_TwCl5_nb1.xlsx")
export(nb_2_fold, "pred_TwCl5_nb2.xlsx")
export(nb_3_fold, "pred_TwCl5_nb3.xlsx")
...
export(conf_matrix_SVM, file = "TwCl5_confmatrix_SVM.xlsx")
...
export(svm_1_fold, "pred_TwCl5_svm1.xlsx")
export(svm_2_fold, "pred_TwCl5_svm2.xlsx")
export(svm_3_fold, "pred_TwCl5_svm3.xlsx")
...
#Performance
conf_matrix_RF <- calculateConfusionMatrix(cv_RF$pred, relative = TRUE)
conf_matrix_RF <- cbind(rownames(conf_matrix_RF$result), as_tibble(conf_matrix_RF$result))
export(conf_matrix_RF, file = "TwCl5_confmatrix_RF.xlsx")
...
export(rf_1_fold, "pred_TwCl5_rf1.xlsx")
export(rf_2_fold, "pred_TwCl5_rf2.xlsx")
export(rf_3_fold, "pred_TwCl5_rf3.xlsx")
