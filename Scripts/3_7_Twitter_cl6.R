...
tweets_cleav6_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav6 <- rbind(y, twitter_cleav6)
}

frq(twitter_cleav6$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav6 <- twitter_cleav6 %>% select(-cleavage_6_10, -cleavage_6)

# Create DFM  -----------------------------------------------------------------
twitter_cleav6 <- corpus(twitter_cleav6)

# Further cleaning procedure
twitter_cleav62 <- tokens(twitter_cleav6 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav62 <- tokens_remove(twitter_cleav62, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav62 <- tokens_remove(twitter_cleav62, c("0*"))
twitter_cleav62 <- tokens_wordstem(twitter_cleav62, language = "german")
twitter_cleav62 <- dfm(twitter_cleav62)
topfeatures(twitter_cleav62 , 1000)  # Check Topfeatures and in how many docs they are

# Trimming 1: Remove features that appear in less than 15 Tweets
twitter_cleav62 <- dfm_trim(twitter_cleav62 , min_docfreq = 15, verbose=TRUE)
twitter_cleav62  <- dfm_remove(twitter_cleav62 , min_nchar = 2)
topfeatures(twitter_cleav62 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav62[ntoken(twitter_cleav62) == 0,]
twitter_cleav62  <-  twitter_cleav62[ntoken(twitter_cleav62) != 0,]
twitter_cleav62[ntoken(twitter_cleav62) == 0,]



twitter_cleav6_df <- twitter_cleav62 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav62)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav6_df) <- make.names(names(twitter_cleav6_df))

frq(twitter_cleav6_df$ideology_scale7)
tweets_cleav6_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav6_df, target = "ideology_scale7")
...
export(conf_matrix_knn, file = "TwCl6_confmatrix_knn.xlsx")
...
export(knn_1_fold, "pred_TwCl6_knn1.xlsx")
export(knn_2_fold, "pred_TwCl6_knn2.xlsx")
export(knn_3_fold, "pred_TwCl6_knn3.xlsx")
...
export(conf_matrix_NB, file = "TwCl6_confmatrix_NB.xlsx")
...
export(nb_1_fold, "pred_TwCl6_nb1.xlsx")
export(nb_2_fold, "pred_TwCl6_nb2.xlsx")
export(nb_3_fold, "pred_TwCl6_nb3.xlsx")
# SVM ---------------------------------------------------------------------
...
export(conf_matrix_SVM, file = "TwCl6_confmatrix_SVM.xlsx")
...
export(svm_1_fold, "pred_TwCl6_svm1.xlsx")
export(svm_2_fold, "pred_TwCl6_svm2.xlsx")
export(svm_3_fold, "pred_TwCl6_svm3.xlsx")
...
export(conf_matrix_RF, file = "TwCl6_confmatrix_RF.xlsx")
...
export(rf_1_fold, "pred_TwCl6_rf1.xlsx")
export(rf_2_fold, "pred_TwCl6_rf2.xlsx")
export(rf_3_fold, "pred_TwCl6_rf3.xlsx")