...
tweets_cleav7_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav7 <- rbind(y, twitter_cleav7)
}

frq(twitter_cleav7$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav7 <- twitter_cleav7 %>% select(-cleavage_7_10, -cleavage_7)

# Create DFM  -----------------------------------------------------------------
twitter_cleav7 <- corpus(twitter_cleav7)

# Further cleaning procedure
twitter_cleav72 <- tokens(twitter_cleav7 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav72 <- tokens_remove(twitter_cleav72, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav72 <- tokens_remove(twitter_cleav72, c("0*"))
twitter_cleav72 <- tokens_wordstem(twitter_cleav72, language = "german")
twitter_cleav72 <- dfm(twitter_cleav72)
topfeatures(twitter_cleav72 , 1000)  # Check Topfeatures and in how many docs they are

# Trimming 1: Remove features that appear in less than 15 Tweets
twitter_cleav72 <- dfm_trim(twitter_cleav72 , min_docfreq = 15, verbose=TRUE)
twitter_cleav72  <- dfm_remove(twitter_cleav72 , min_nchar = 2)
topfeatures(twitter_cleav72 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav72[ntoken(twitter_cleav72) == 0,]
twitter_cleav72  <-  twitter_cleav72[ntoken(twitter_cleav72) != 0,]
twitter_cleav72[ntoken(twitter_cleav72) == 0,]



twitter_cleav7_df <- twitter_cleav72 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav72)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav7_df) <- make.names(names(twitter_cleav7_df))

frq(twitter_cleav7_df$ideology_scale7)
tweets_cleav7_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav7_df, target = "ideology_scale7")
...
export(conf_matrix_knn, file = "TwCl7_confmatrix_knn.xlsx")
...
export(knn_1_fold, "pred_TwCl7_knn1.xlsx")
export(knn_2_fold, "pred_TwCl7_knn2.xlsx")
export(knn_3_fold, "pred_TwCl7_knn3.xlsx")
...
export(conf_matrix_NB, file = "TwCl7_confmatrix_NB.xlsx")
...
export(nb_1_fold, "pred_TwCl7_nb1.xlsx")
export(nb_2_fold, "pred_TwCl7_nb2.xlsx")
export(nb_3_fold, "pred_TwCl7_nb3.xlsx")
...
export(conf_matrix_SVM, file = "TwCl7_confmatrix_SVM.xlsx")
...
export(svm_1_fold, "pred_TwCl7_svm1.xlsx")
export(svm_2_fold, "pred_TwCl7_svm2.xlsx")
export(svm_3_fold, "pred_TwCl7_svm3.xlsx")
...
export(conf_matrix_RF, file = "TwCl7_confmatrix_RF.xlsx")
...
export(rf_1_fold, "pred_TwCl7_rf1.xlsx")
export(rf_2_fold, "pred_TwCl7_rf2.xlsx")
export(rf_3_fold, "pred_TwCl7_rf3.xlsx")