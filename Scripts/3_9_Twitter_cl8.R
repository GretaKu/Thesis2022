...
# removing "umlaute", because MLR package does not like umlaut in variable name
x$text <- gsub(pattern = '[ä]', replacement = "ae", x$text)
x$text <- gsub(pattern = '[ü]', replacement = "ue", x$text)
x$text <- gsub(pattern = '[ö]', replacement = "oe", x$text)
x$text <- gsub(pattern = '[ö]', replacement = "oe", x$text)
x$text <- gsub(pattern = '[à]', replacement = "a", x$text)
x$text <- gsub(pattern = '[â]', replacement = "a", x$text)
x$text <- gsub(pattern = '[é]', replacement = "e", x$text)
x$text <- gsub(pattern = '[è]', replacement = "e", x$text)
x$text <- gsub(pattern = '[ê]', replacement = "e", x$text)

# 2. Balance to reduce size (7-point-scale)  ---------------------------
x_7 <- x %>% mutate(
  ideology_scale7 = ifelse(cleavage_8_10 == 9 | cleavage_8_10 == 10, "7:Strong Agree",
                           ifelse(cleavage_8_10 == 7 | cleavage_8_10 == 8, "6:Agree",
                                  ifelse(cleavage_8_10 == 6, "5:Moderate Agree",
                                         ifelse(cleavage_8_10 == 5, "4:Neutral",
                                                ifelse(cleavage_8_10 == 4, "3:Moderate Disagree",
                                                       ifelse(cleavage_8_10 == 3 | cleavage_8_10 == 2, "2:Disagree",
                                                              ifelse(cleavage_8_10 == 1 | cleavage_8_10 == 0, "1:Strong Disagree", NA))))))),
  ideology_scale7 = factor(ideology_scale7))

frq(x_7$cleavage_8_10)
...
tweets_cleav8_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav8 <- rbind(y, twitter_cleav8)
}

frq(twitter_cleav8$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav8 <- twitter_cleav8 %>% select(-cleavage_8_10, -cleavage_8)

# Create DFM  -----------------------------------------------------------------
twitter_cleav8 <- corpus(twitter_cleav8)

# Further cleaning procedure
twitter_cleav82 <- tokens(twitter_cleav8 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav82 <- tokens_remove(twitter_cleav82, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav82 <- tokens_remove(twitter_cleav82, c("0*"))
twitter_cleav82 <- tokens_wordstem(twitter_cleav82, language = "german")
twitter_cleav82 <- dfm(twitter_cleav82)
topfeatures(twitter_cleav82 , 1000)  

# Trimming 1: Remove features that appear in less than 5 Tweets
twitter_cleav82 <- dfm_trim(twitter_cleav82 , min_docfreq = 5, verbose=TRUE)
twitter_cleav82  <- dfm_remove(twitter_cleav82 , min_nchar = 2)
topfeatures(twitter_cleav82 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav82[ntoken(twitter_cleav82) == 0,]
twitter_cleav82  <-  twitter_cleav82[ntoken(twitter_cleav82) != 0,]
twitter_cleav82[ntoken(twitter_cleav82) == 0,]



twitter_cleav8_df <- twitter_cleav82 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav82)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav8_df) <- make.names(names(twitter_cleav8_df))

frq(twitter_cleav8_df$ideology_scale7)
tweets_cleav8_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav8_df, target = "ideology_scale7")
...
export(conf_matrix_knn, file = "TwCl8_confmatrix_knn.xlsx")
...
export(knn_1_fold, "pred_TwCl8_knn1.xlsx")
export(knn_2_fold, "pred_TwCl8_knn2.xlsx")
export(knn_3_fold, "pred_TwCl8_knn3.xlsx")
...
export(conf_matrix_NB, file = "TwCl8_confmatrix_NB.xlsx")
...
export(nb_1_fold, "pred_TwCl8_nb1.xlsx")
export(nb_2_fold, "pred_TwCl8_nb2.xlsx")
export(nb_3_fold, "pred_TwCl8_nb3.xlsx")
...
export(conf_matrix_SVM, file = "TwCl8_confmatrix_SVM.xlsx")
...
export(svm_1_fold, "pred_TwCl8_svm1.xlsx")
export(svm_2_fold, "pred_TwCl8_svm2.xlsx")
export(svm_3_fold, "pred_TwCl8_svm3.xlsx")
...
export(conf_matrix_RF, file = "TwCl8_confmatrix_RF.xlsx")
...
export(rf_1_fold, "pred_TwCl8_rf1.xlsx")
export(rf_2_fold, "pred_TwCl8_rf2.xlsx")
export(rf_3_fold, "pred_TwCl8_rf3.xlsx")