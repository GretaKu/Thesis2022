...
tweets_cleav1_doc_7 <- data.frame(frq(x_7$ideology_scale7))
...
for (i in vec) {
  ...
  twitter_cleav1 <- rbind(y, twitter_cleav1)
}

frq(twitter_cleav1$ideology_scale7)

#Removing extra scale to avoid error in coding
twitter_cleav1 <- twitter_cleav1 %>% select(-cleavage_1_10, -cleavage_1)

# Create DFM  -----------------------------------------------------------------
twitter_cleav1 <- corpus(twitter_cleav1)

# Further cleaning procedure
twitter_cleav12 <- tokens(twitter_cleav1 , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                          split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
twitter_cleav12 <- tokens_remove(twitter_cleav12, c(stopwords("en"), stopwords("de"), "dass", "مع", "ممتاز"))

twitter_cleav12 <- tokens_remove(twitter_cleav12, c("0*"))
twitter_cleav12 <- tokens_wordstem(twitter_cleav12, language = "german")
twitter_cleav12 <- dfm(twitter_cleav12)
topfeatures(twitter_cleav12 , 1000)  # Check Topfeatures and in how many docs they are

# Trimming 1: Remove features that appear in less than 5 Tweets
twitter_cleav12 <- dfm_trim(twitter_cleav12 , min_docfreq = 5, verbose=TRUE)
twitter_cleav12  <- dfm_remove(twitter_cleav12 , min_nchar = 2)
topfeatures(twitter_cleav12 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
twitter_cleav12[ntoken(twitter_cleav12) == 0,]
twitter_cleav12  <-  twitter_cleav12[ntoken(twitter_cleav12) != 0,]
twitter_cleav12[ntoken(twitter_cleav12) == 0,]



twitter_cleav1_df <- twitter_cleav12 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(twitter_cleav12)) %>% 
  select(-doc_id)


# solving problem of features starting with digits eg 2x
names(twitter_cleav1_df) <- make.names(names(twitter_cleav1_df))

frq(twitter_cleav1_df$ideology_scale7)
tweets_cleav1_fea_7 <- data.frame(frq(x$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = twitter_cleav1_df, target = "ideology_scale7")
...
export(conf_matrix_knn, file = "TwCl1_confmatrix_knn.xlsx")
...
export(knn_1_fold, "pred_TwCl1_knn1.xlsx")
export(knn_2_fold, "pred_TwCl1_knn2.xlsx")
export(knn_3_fold, "pred_TwCl1_knn3.xlsx")
...
export(conf_matrix_NB, file = "TwCl1_confmatrix_NB.xlsx")
...
export(nb_1_fold, "pred_TwCl1_nb1.xlsx")
export(nb_2_fold, "pred_TwCl1_nb2.xlsx")
export(nb_3_fold, "pred_TwCl1_nb3.xlsx")
...
export(conf_matrix_SVM, file = "TwCl1_confmatrix_SVM.xlsx")
...
export(svm_1_fold, "pred_TwCl1_svm1.xlsx")
export(svm_2_fold, "pred_TwCl1_svm2.xlsx")
export(svm_3_fold, "pred_TwCl1_svm3.xlsx")
...
export(conf_matrix_RF, file = "TwCl1_confmatrix_RF.xlsx")
...
export(rf_1_fold, "pred_TwCl1_rf1.xlsx")
export(rf_2_fold, "pred_TwCl1_rf2.xlsx")
export(rf_3_fold, "pred_TwCl1_rf3.xlsx")
