...
load("transcript_selects.RData")
x <- x %>% filter(Text3 != "") %>% 
  rowid_to_column("id_text") #rownames to connect results to text

export(x, "Transcript_WithText_QUAL.xlsx")
...
transcript_selects_doc_7 <- data.frame(frq(x_7$ideology_scale7))

transcript_selects = NULL
y = NULL
...
for (i in vec) {
  ...
  transcript_selects <- rbind(y, transcript_selects)
}

frq(transcript_selects$ideology_scale7)

#Removing extra scale to avoid error in coding
transcript_selects <- transcript_selects %>% select(-ideology_scale, -Text3)

# Create DFM  -----------------------------------------------------------------
transcript_selects <- corpus(transcript_selects)

# Further cleaning procedure
transcript_selects2 <- tokens(transcript_selects , remove_punct = TRUE, remove_numbers=TRUE, remove_symbols = TRUE, 
                           split_hyphens = TRUE, remove_separators = TRUE, remove_url = TRUE)
transcript_selects2 <- tokens_remove(transcript_selects2, c(stopwords("de"), "dass"))

transcript_selects2 <- tokens_remove(transcript_selects2, c("0*"))
transcript_selects2 <- tokens_wordstem(transcript_selects2, language = "german")
transcript_selects2 <- dfm(transcript_selects2)
topfeatures(transcript_selects2 , 1000)  # 20 top words

# Trimming 1: Remove features that only appear in 2 Tweet
transcript_selects2 <- dfm_trim(transcript_selects2 , min_docfreq = 15, verbose=TRUE)
transcript_selects2  <- dfm_remove(transcript_selects2 , min_nchar = 2)
topfeatures(transcript_selects2 , 20)  # 20 top words

# Trimming 2: Remove doc with only 0
transcript_selects2[ntoken(transcript_selects2) == 0,]
transcript_selects2  <-  transcript_selects2[ntoken(transcript_selects2) != 0,]
transcript_selects2[ntoken(transcript_selects2) == 0,]


transcript_selects_df <- transcript_selects2 %>% 
  quanteda::convert(to = "data.frame") %>% 
  cbind(docvars(transcript_selects2)) %>% 
  rowid_to_column("id_pred") #adding ID to link Pred results after prediction

export(transcript_selects_df, "Transcript_PrePrediction_QUAL.xlsx")


transcript_selects_df <- transcript_selects_df %>% 
  select(-doc_id, -id_pred, -id_text)

# solving problem of features starting with digits eg 2x
names(transcript_selects_df) <- make.names(names(transcript_selects_df))

frq(transcript_selects_df$ideology_scale7)
transcript_selects_fea_7 <- data.frame(frq(transcript_selects_df$ideology_scale7))

# Task
ml_task <- makeClassifTask(data = transcript_selects_df, target = "ideology_scale7")
...
export(knn_1_fold, "pred_TrSe_knn1.xlsx")
export(knn_2_fold, "pred_TrSe_knn2.xlsx")
export(knn_3_fold, "pred_TrSe_knn3.xlsx")
...
export(nb_1_fold, "pred_TrSe_nb1.xlsx")
export(nb_2_fold, "pred_TrSe_nb2.xlsx")
export(nb_3_fold, "pred_TrSe_nb3.xlsx")
...
export(svm_1_fold, "pred_TrSe_svm1.xlsx")
export(svm_2_fold, "pred_TrSe_svm2.xlsx")
export(svm_3_fold, "pred_TrSe_svm3.xlsx")
...
export(rf_1_fold, "pred_TrSe_rf1.xlsx")
export(rf_2_fold, "pred_TrSe_rf2.xlsx")
export(rf_3_fold, "pred_TrSe_rf3.xlsx")